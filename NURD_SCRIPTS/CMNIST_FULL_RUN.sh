...
do
        ...
        do
            # if [[ $BORDER != 6 || $RHO_TEST != 9 ]]; then
                for SEED in 1000 2000 3000 4000 5000 6000 7000 8000 9000 500
                    ...
                    PREFIX="RWNURD_${method}_${DATASET}_BS${BS}_seed${SEED}_RHOTEST0${RHO_TEST}_BORDER${BORDER}_DEF_FINAL"                    ARGS_STRING="--prefix=$PREFIX --dataset=$DATASET --workers=0 --dist_epochs=$DIST_EPOCHS --nr_epochs=$NR_EPOCHS --nr_batch_size=$BS --dist_batch_size=$BS --seed=$SEED --rho=0.9 --img_side=$IMGSIZE --rho_test=0.${RHO_TEST} --pred_model_type=$MODEL --weight_model_type=$MODEL --num_folds=$NUM_FOLDS --nr_strategy=$method --debug=2"

                    # WEIGHT MODEL COMMAND
                    COMMAND_STRING="nurd_reweighting.py $ARGS_STRING --nr_only >  SBATCH_LOGS/weights_$PREFIX.log"
    ...
    eval '$PYTHON_STRING $COMMAND_STRING'

    eval '$PYTHON_STRING nurd_reweighting.py $ARGS_STRING --add_pred_suffix=_LAM1_FRAC1_RR1 --load_weights --lambda_=1 --max_lambda_=1 --frac_phi_steps=1 > SBATCH_LOGS/${PREFIX}_LAM1_FRAC1_RR1.log'
