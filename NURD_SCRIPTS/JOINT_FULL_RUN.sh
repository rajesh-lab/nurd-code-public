...
# ERM
eval '$PYTHON_STRING nurd_reweighting.py $ARGS_STRING --add_pred_suffix=_DEFLR_distWD0_ERM --load_weights --lambda_=0 --max_lambda_=0 --frac_phi_steps=0 --randomrestart=-1 --exact --coeff=0.5 > SBATCH_LOGS/${PREFIX}_DEFLR_distWD0_ERM.log'

# 1 epoch per pred update, with randomrestart=1
eval '$PYTHON_STRING nurd_reweighting.py $ARGS_STRING --add_pred_suffix=_DEFLR_distWD0_LAM1_FRAC1_RR1_LONG --load_weights --lambda_=1 --max_lambda_=1 --frac_phi_steps=1 --randomrestart=1 > SBATCH_LOGS/${PREFIX}_DEFLR_distWD0_LAM1_FRAC1_RR1_LONG.log'

# 2 epoch per pred update, with randomrestart=1
eval '$PYTHON_STRING nurd_reweighting.py $ARGS_STRING --add_pred_suffix=_DEFLR_distWD0_LAM1_FRAC2_RR1_LONG --load_weights --lambda_=1 --max_lambda_=1 --frac_phi_steps=2 --randomrestart=1 > SBATCH_LOGS/${PREFIX}_DEFLR_distWD0_LAM1_FRAC2_RR1_LONG.log'
                    """
