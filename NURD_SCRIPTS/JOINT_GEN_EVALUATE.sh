            ...
            do
                # if [[ $BORDER != 6 || $RHO_TEST != 9 ]]; then
                    for SEED in 1000 2000 3000 4000 5000 6000 7000 8000 9000 500
                        ...
                        PREFIX="GENNURD_${method}_${DATASET}_BAL${DATABALANCETYPE}_BS${BS}_seed${SEED}_RHOTEST0${RHO_TEST}_BORDER${BORDER}_noBN"
                        ARGS_STRING="--prefix=$PREFIX --dataset=$DATASET --workers=0 --dist_epochs=$DIST_EPOCHS --nr_epochs=$NR_EPOCHS --nr_batch_size=$BS --dist_batch_size=$BS --seed=$SEED --rho=0.9 --img_side=$IMGSIZE --label_balance_method=$DATABALANCETYPE --rho_test=0.${RHO_TEST} --border=$BORDER --pred_model_type=$MODEL --weight_model_type=$MODEL --num_folds=$NUM_FOLDS --theta_lr=0.001 --gamma_lr=0.001 --phi_lr=0.001 --nr_lr=0.001 --dist_decay=0.0 --phi_decay=0.0 --nr_decay=0.01 --nr_strategy=$method --exact --coeff=0.5 --debug=2"

                    COMMAND_STRING="""

# 1 epoch per pred update, with randomrestart=1
# eval '$PYTHON_STRING evaluate_model.py $ARGS_STRING --add_pred_suffix=_DEFLR_distWD0_LAM1_FRAC1_RR1_LONG --load_final_model --add_final_results_suffix=SAVED --lambda_=1 --max_lambda_=1 --frac_phi_steps=1 --randomrestart=1'

# 2 epoch per pred update, with randomrestart=1
# eval '$PYTHON_STRING evaluate_model.py $ARGS_STRING --add_pred_suffix=_DEFLR_distWD0_LAM1_FRAC2_RR1_LONG --load_final_model --add_final_results_suffix=SAVED --lambda_=1 --max_lambda_=1 --frac_phi_steps=2 --randomrestart=1 
                    """
